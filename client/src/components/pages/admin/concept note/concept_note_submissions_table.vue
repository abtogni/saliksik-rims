<template>
  <div style="padding-bottom: 0.833rem">
    ...
    <v-data-table :headers="concept_note_header" :items="formattedConceptNote" :search="search" density="comfortable"
      class="b">
      <!-- @vue-skip -->
      <template v-slot:item="{ item: research }">
        <tr style="align-items: center; white-space: nowrap">
          <td class="truncate-table">
            <v-btn variant="text" density="comfortable" icon="mdi-account-multiple-outline" style="margin-right: 1rem">
              <v-icon></v-icon>
              <v-tooltip activator="parent" location="bottom" class="tooltip-list">
                <div class="bold-upper">Researchers</div>
                <div v-for="(leader, index) in research.researchLeaders" :key="index">
                  {{ leader.name }}
                </div>
              </v-tooltip>
            </v-btn>
            <a :href="`/administrator/concept_note/${research._id}`">{{
