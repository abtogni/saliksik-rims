<template>
  <v-card
    ...
    v-for="(schedule_group, date) in grouped_schedules"
    :key="date"
    class="body"
  >
    ...
    <v-card-text class="content">
      <v-card variant="outlined" class="card-style">
        <v-expansion-panels>
          <v-expansion-panel
            v-for="(schedule, index) in schedule_group"
            :key="schedule._id"
          >
            <v-expansion-panel-title
              class="title expansion-title-body b"
              v-if="should_display_time(schedule, index, schedule_group)"
            >
              <v-btn variant="text" icon="mdi-account-multiple-outline">
                <v-badge
                  color="info"
                  :content="schedule.panelists.length"
                  floating
                >
                  <v-icon></v-icon>
                </v-badge>
                ...
                >
                  ...
                  <div
                    v-for="panelist in schedule.panelists"
                    :key="panelist._id"
                  >
                    {{ panelist.name }}
                  </div>
              ...
              <v-btn variant="text" icon="mdi-note-outline">
                <v-badge
                  color="info"
                  :content="schedule.presentations.length"
                  floating
                >
                  <v-icon />
                </v-badge>
                ...
                >
                  ...
                  <div v-for="p in schedule.presentations" :key="p._id">
                    {{
                      researches.find(
                        (research: any) => research.key === p.researchID,
                      )?.name
                    }}
                  </div>
                </v-tooltip>
              ...
              <v-btn type="submit" flat icon="mdi-calendar-edit-outline" style="background-color: transparent;">
                      <v-icon></v-icon>
                      <v-tooltip activator="parent" location="bottom"
                        >Change Schedule
                      </v-tooltip>
            ...
            <v-expansion-panel-text>
              ...
              <schedule_researches_preview
                :presentations="schedule.presentations"
                :researches="researches"
                :users="users"
              />
              <br />
              <schedule_panelist_preview
...
const { schedule, researches, users } = defineProps([
  "schedule",
  "researches",
  "users",
]);

const sortedSchedules = [...schedule].sort((a: any, b: any) =>
  moment(a.dateAndTime).diff(moment(b.dateAndTime)),
);

const grouped_schedules = sortedSchedules.reduce((acc: any, curr: any) => {
