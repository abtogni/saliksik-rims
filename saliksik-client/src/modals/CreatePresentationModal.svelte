<script lang="ts">
  ...
  import { CalendarPlusSolid, CalendarMonthOutline, MessageCaptionOutline, FileCirclePlusOutline, MinusOutline, CirclePlusOutline, FileLinesOutline } from "flowbite-svelte-icons";
  import { researches } from "../components/store";
  import { onMount } from "svelte";
  

  var researchTitles: any[] = [], researchList: any, addedResearches: any;
  let presentationDate: any;
...
<form class="flex flex-col gap-2" on:submit={submit}>
  ...
  <div class="grid grid-flow-row gap-2 border-b pb-5">
    <div class="flex justify-start items-center gap-2">
      ...
      <P for="researchLeaders" weight="bold" size="base" class="text-gray-700">Date</P>
    </div>
    <div class="flex justify-start items-center gap-2 px-2 py-0">
      <CalendarMonthOutline size="sm" class="text-blue-700" />
      <DateInput bind:value={presentationDate} format="MM-dd-yyyy" placeholder="12-31-2024" required --date-picker-highlight-border="#b43403" --date-picker-highlight-shadow="none" --date-input-width="100%" class="w-full rounded-md text-gray-500  " />
    </div>
  </div>
  ...
  <div class="grid grid-flow-row gap-2 border-b pb-5">
    ...
    <div class="flex justify-start items-center gap-2 p-2 rounded-md w-full">
      <div class="gap-2 w-full">
        <MultiSelect required items={researchList} bind:value={researchTitles} size="sm" />
      </div>
      ...
      {#each addedResearches as r}
          <div class="flex justify-between items-center p-2 rounded-md hover:bg-blue-50">
            <div class="flex justify-between items-center gap-2">
              ...
              <P for="researchIDs" weight="semibold" size="sm" class="text-gray-500">
                <Input type="text" id="researchIDs" name="researchIDs" value={r._id} readonly />
                </P>
            </div>
